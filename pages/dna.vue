<template>
    <TresCanvas window-size clear-color="black">
        <TresPerspectiveCamera :position="[0, 0, 0.3]" />
        <OrbitControls />
        <TresPointLight :position="[10, 10, 10]" />
        <TresAmbientLight />
        <template v-for="(curve, index) in brainCurves" :key="index">
            <TresMesh>
                <TresTubeGeometry :args="[curve, 64, 0.001, 2, false]" />
                <TresShaderMaterial ref="bufferRef" v-bind="shader" />
            </TresMesh>
        </template>
    </TresCanvas>
</template>
...
onLoop(({ elapsed }) => {
  if (bufferRef.value) {
    bufferRef.value.forEach((item) => {
        item.uniforms.time.value = elapsed;
    })
  }
})
...
scene.economics[0].paths.forEach((path) => {
    let points = [];
